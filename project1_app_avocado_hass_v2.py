...
# 4. Predict prices by Time series
    ...
    if __name__=='__main__':
        model_pre = pickle.load(open('model/Problem2_3_model_pre_processing.sav', 'rb'))

    # content
    st.header('PREDICT PRICES BY TIME SERIES')

    # sidebar - input
    num_month = st.sidebar.number_input("Choose the number of month", value = 1)
    ...
    if st.sidebar.button("Show predict results"):
        ...
        uploaded_data = uploaded_data.loc[uploaded_data['region'] != "TotalUS",:]
        uploaded_data = uploaded_data.loc[:, ~uploaded_data.columns.str.contains('^Unnamed')]
        clean_data = model_pre.transform(uploaded_data)
        m, forecast = FP_California_organic_forecast(clean_data, num_month)
    # content
        fig = m.plot(forecast) 
        fig.show()
